...
public class OpenPlanActivity extends AppCompatActivity {
    ...
    public void onBackPressed() {
        ...
        */
        ...
        if (title.equals("") && desc.equals("")) {
            ...
            dbHandler.updateData(idString, title,
                tag,desc,loc,12);
        }
        ...
        if(checkBox.isChecked()) {
            ...
            if (chkRepeat.isChecked()){
                ...
                else if (timeCurrent.getTimeInMillis() == getAlarmTime() && repNum == repCurrent && spin_val == spinCurrent) {
                    startActivity(parentIntent);
                    // If both are the same then there is nothing to do
                }
                else{
                    dbHandler.updateReminder(idString, getAlarmTime(), repNum, spin_val);
                ...
                if (time.equals("") || date.equals("") || timeCurrent.getTimeInMillis() == getAlarmTime()) {
                    Toast.makeText(OpenPlanActivity.this, time, Toast.LENGTH_SHORT).show();
                    startActivity(parentIntent);
                }
                else{
                    dbHandler.updateReminder(idString, getAlarmTime(), repNum, spin_val);
    ...
    public void fillEditView(int id) {
        ...
        Cursor res2 = dbHandler.getIdReminder(id);
        if (res2.getCount() == 0)
            return;
        while (res2.moveToNext()) {
            timeCurrent.setTimeInMillis(res2.getLong(1));
            repText.setText(res2.getInt(2));
            repCurrent = res2.getInt(2);
            spinner.setPromptId(res2.getInt(3));
            spin_val = spinCurrent = res2.getInt(3);
            //This is only for display, the rest is code.
            //Time is in milliseconds.
        }
        pickYear = timeCurrent.get(Calendar.YEAR);
        pickMonth = timeCurrent.get(Calendar.MONTH);
        pickDay = timeCurrent.get(Calendar.DAY_OF_MONTH);
        pickHour = timeCurrent.get(Calendar.HOUR_OF_DAY);
        pickMinute = timeCurrent.get(Calendar.MINUTE);
        String theTime = String.format("%02d", pickHour)+" : " + String.format("%02d", pickMinute);
        String theDate = pickDay + " / " + pickMonth + " / " + pickYear;
        viewTime.setText(theTime);
        viewDate.setText(theDate);

    }


    // *********************    REMEMBER TO REMOVE NOTIFICATIONS    ***************************
    ...
    protected Dialog CreateDialog(int id){
        ...
        if(id == 1){
            return new TimePickerDialog(OpenPlanActivity.this, tpListener, pickHour, pickMinute, true);
        }
        else if (id == 2){
            DatePickerDialog datePickerDialog = new DatePickerDialog(OpenPlanActivity.this, dpListener, pickYear, pickMonth, pickDay);
